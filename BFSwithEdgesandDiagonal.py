...
def BFS(startNode, endNode):
    ...
    while startFlag and queue:
        ...
        if searchFlag:
            ...
            if grid[x+1][y-1].col > 0 and grid[x+1][y-1] not in visited and not grid[x+1][y-1].is_obstacle(): # UP-RIGHT
                visited.add(grid[x+1][y-1])
                queue.append(grid[x+1][y-1])
                grid[x+1][y-1].prev.append(grid[x][y])

            if grid[x+1][y+1].col < 29 and grid[x+1][y+1] not in visited and not grid[x+1][y+1].is_obstacle(): # DOWN-RIGHT            
                visited.add(grid[x+1][y+1])
                queue.append(grid[x+1][y+1])
                grid[x+1][y+1].prev.append(grid[x][y])

            if grid[x-1][y-1].row < 39 and grid[x-1][y-1] not in visited and not grid[x-1][y-1].is_obstacle(): # UP-LEFT          
                visited.add(grid[x-1][y-1])
                queue.append(grid[x-1][y-1])            
                grid[x-1][y-1].prev.append(grid[x][y])

            if grid[x-1][y+1].row > 0 and grid[x-1][y+1] not in visited and not grid[x-1][y+1].is_obstacle(): # DOWN-LEFT
                visited.add(grid[x-1][y+1])
                queue.append(grid[x-1][y+1])
                grid[x-1][y+1].prev.append(grid[x][y]) 
...
def edges():
    for i in range(30):
        grid[0][i].make_obstacle()
        grid[39][i].make_obstacle()
    for j in range(40):
        grid[j][0].make_obstacle()
        grid[j][29].make_obstacle()        
